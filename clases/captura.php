    ...
    if ($id > 0) {
        ...
        if ($productos != null) {
            foreach ($productos as $clave => $cantidad) {
                $sql = $con->prepare("SELECT id_Prod, nombre, precio, descuento FROM productos WHERE id_Prod=? AND activo=1");
                $sql->execute([$clave]);
                ...
                $sql_insert = $con->prepare("INSERT INTO detalle_compra (id_com, id_prod, nombre, precio, 
                cantidad) VALUES (?,?,?,?,?)");
